...
function openTab(evt, tabName) {
    ...
    // Actualizar pastilla de sección
    ...
    // Actualizar minibarra de misión
...
function renderAchievements() {
    ...
    grid.innerHTML = ACHIEVEMENTS.map(a => {
        ...
        return `<div class="ach-item ${unlocked ? "unlocked" : "locked"}">
            <div class="left"><div class="icon"><i class="fas ${unlocked ? "fa-trophy" : "fa-lock"}"></i></div>
            <div class="meta"><div class="title">${a.title}</div><div class="desc">${a.desc}</div></div></div>
            <div class="badge">${unlocked ? "Desbloqueado" : a.xp + " XP"}</div></div>`;
    }).join("");
}
...
function checkPractice() {
    ...
    if (!PRACTICE_STATE || !ansRaw) { feedback && (feedback.textContent = "Escribe una respuesta."); return; }
...
// ========== NUEVAS FUNCIONES PARA ÁLGEBRA DE BOOLE ==========
...
// Misión 3: Simplificación con mapa K básico
function simplifyWithKMap() {
    ...
    if (!out) return;
    // Ejemplo fijo para 3 variables
    const minterms = [1, 2, 5, 7];
    const simplified = "A·C + B·¬C";
    kmapDiv.innerHTML = `Mapa K (3 vars) para Σm(1,2,5,7):<br><pre>   BC
A  00 01 11 10
0   0  1  0  1
1   0  0  1  1</pre>`;
    out.textContent = `Expresión simplificada: ${simplified}`;
...
function optimizeCondition() {
    ...
    let optimized = raw.replace(/\(([A-Za-z]+)\s*\|\|\s*\1\)/g, "$1")
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("resetProgressBtn")?.addEventListener("click", () => {
        if (confirm("¿Reiniciar progreso?")) { ["mc_xp","mc_level","mc_done","mc_streak","mc_ach","mc_xp_keys","mc_missions"].forEach(k => localStorage.removeItem(k)); location.reload(); }
    });
